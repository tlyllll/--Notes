...
## 线程和进程
进程是对运行时程序的封装，是**系统**进行**资源调度和分配**的的**基本单位**，实现了**操作系统的并发**；

线程是进程的**子任务**，是**CPU调度和分派的基本单位**，用于保证**程序的实时性**，实现**进程内部的并发**；

线程是操作系统可识别的**最小执行和调度单位**。

每个线程都**独自占用一个虚拟处理器**：独自的寄存器组，指令计数器和处理器状态。

每个线程完成不同的任务，但是**共享同一地址空间**（也就是同样的动态内存，映射文件，目标代码等等），打开的文件队列和其他内核资源。

### 区别
1. 一个线程只对应一个进程，一个进程可能有很多线程
2. 进程拥有独立的内存单元，而多个线程共享一个进程的内存
