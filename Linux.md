...
* 10. [分时系统/实时系统](#-1)
	...
	* 10.3. [比较](#-1)
* 11. [操作系统调度算法](#-1)
	* 11.1. [1. **FCFS** 先来先服务](#FCFS)
	* 11.2. [2. **SJF/SPF** 短作业/短进程优先算法](#SJFSPF)
	* 11.3. [3. **RR** 时间片轮转调度算法](#RR)
* 12. [页面置换算法](#-1)
	* 12.1. [1. **OPT** 最佳置换算法](#OPT)
	* 12.2. [2. **FIFO** 先进先出置换算法](#FIFO)
	* 12.3. [3. **LRU** 最近最久未使用置换算法](#LRU)
...
##  2. <a name='Shell'></a>常用Shell

###  2.1. <a name='grep'></a>grep
...
###  6.2. <a name='-1'></a>2. 消息队列
进程A --【数据块】-->进程B

避免命名管道的同步和阻塞问题

缺点：数据块最大长度有限制 + 如果频繁通信相当于间接拷贝进程，比较费时+要注意上次没读完的数据
###  6.3. <a name='-1'></a>3. 信号量 【同步/互斥】
同步 
```
S1: 
signal(mutex);
...
// wait 和 signal成对出现
```
###  6.4. <a name='-1'></a>4. 共享内存
让2个进程各自拿出一块虚拟内存空间映射到同一物理内存

**优点**：解决拷贝所消耗的时间


###  6.5. <a name='socket'></a>5. socket
##  7. <a name='-1'></a>死锁
**必要条件**：
1. 互斥，该资源一次只能被一个进程使用
...
##  10. <a name='-1'></a>分时系统/实时系统

###  10.1. <a name='-1'></a>分时操作系统
...
##  11. <a name='-1'></a>操作系统调度算法
###  11.1. <a name='FCFS'></a>1. **FCFS** 先来先服务
...
###  11.2. <a name='SJFSPF'></a>2. **SJF/SPF** 短作业/短进程优先算法
...
###  11.3. <a name='RR'></a>3. **RR** 时间片轮转调度算法
...
##  12. <a name='-1'></a>页面置换算法
替换时缺页率+1
###  12.1. <a name='OPT'></a>1. **OPT** 最佳置换算法
